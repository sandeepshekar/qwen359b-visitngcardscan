...
public class FileSelectionService : IFileSelectionService
{
    ...
    public async Task<List<string>> SelectDirectoriesAsync()
    {
        ...
        // Placeholder implementation: In a real scenario, this would interact with WPF's platform services.
        ...
        // NOTE: Replace these hardcoded paths with actual calls to the Windows FilePicker API in a real project.
        selectedPaths.Add(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyPictures), "TestPhotos"));
        selectedPaths.Add(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyVideos), "TestVideos"));
